...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
     ...
     * 也就是我们通过配置websocket的服务地址，并且可以指定是否使用socketJS(前端一般用socketJs来连接服务）
     *
     * @param registry
     */
    @Override
    public void registerStompEndpoints(StompEndpointRegistry registry) {
         ...
         * 2.setAllowedOrigins("*"):允许跨域(*所有链接都允许)
    ...
    /**
     * 输入通道参数配置(因为有jwt，不进行这个处理，无法进行websocket通讯)
     *
     * @param registration
     */
    @Override
    ...
    public void configureMessageBroker(MessageBrokerRegistry registry) {
        //配置代理域，可以配置多个，配置代理目的地前缀为/queue，可以在配置域上向客户端推送消息,客户端监听代理域
